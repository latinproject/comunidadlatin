<?php
	...
	if ($mygroup instanceof ElggGroup && $mygroup->canEdit()) {
		if (check_entity_relationship($who_guid, 'member', $mygroup_guid)) {
			remove_entity_relationship($who_guid, 'member', $mygroup_guid);
		}
		system_message(elgg_echo('group_roles:removed'));
	} else {
