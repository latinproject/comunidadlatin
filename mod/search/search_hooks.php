...
/**
 * po5i: Return default results for searches on groups only by name.
 * para la parte del where se tuvo que crear la funcion search_get_literal_where_sql
 *
 * @param unknown_type $hook
 * @param unknown_type $type
 * @param unknown_type $value
 ...
 * @return unknown_type
 */
function search_groupnames_hook($hook, $type, $value, $params) {
	$db_prefix = elgg_get_config('dbprefix');
	...
	// force into boolean mode because we've having problems with the
	// "if > 50% match 0 sets are returns" problem.
	$where = search_get_literal_where_sql('ge', $fields, $params);
