...
function create_group_entity($guid, $name, $description) {
	...
	if ($row) {
		...
		if ($exists) {
			...
			//$pg_query = "INSERT into editor_bookigroup"
			//	. " (id, name, url_name, description, owner_id, created) values ('$guid', '$name', '$name', '$description', '0', now())";
                        //pg_grupo_escritura($pgQuery);
...
/**
 * Return a list of this group's linked groups.
 *
 * @param int  $group_guid The ID of the container/group.
 * @param int  $limit      The limit
 * @param int  $offset     The offset
 ...
 * @return mixed
 */
function get_linked_groups($group_guid, $limit = 10, $offset = 0, $site_guid = 0, $count = false) {
	...
	$arr1 = elgg_get_entities_from_relationship(array(
	...
	$arr2 = elgg_get_entities_from_relationship(array(
		'relationship' => 'linkedTo',
		'relationship_guid' => $group_guid,
