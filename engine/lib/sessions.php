...
function pg_get_user_password(){
    global $pg_pass;
    $rp = $pg_pass;
    return $rp;
}

function pg_set_user_password($p){
    global $pg_pass;
    $pg_pass = $p;
}

function pg_get_pass64_in_user_entity() {
	global $SESSION;

	if (isset($SESSION)) {
		return base64_encode($SESSION['mip64']);
	}
