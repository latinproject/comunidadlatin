...
//$user = elgg_get_logged_in_user_entity();
    ...
    if ($relationship == 'au_subgroup_of') {
        $pg_guid = (int) elgg_get_logged_in_user_guid();
        $pg_group = get_group_entity_as_row($guid_one);

        if ($pg_guid != 0) {
            if ($pg_group) {
                $pg_url_name = str_replace(" ","-",$pg_group->name);
                $pgQuery = "INSERT INTO editor_bookigroup (id, name, url_name, description, owner_id, created) VALUES ('$pg_group->guid','$pg_group->name', '$pg_url_name', '$pg_group->description', '$pg_guid', now())";
                pg_crea_grupo_escritura($pgQuery);
                $pgQuery = "INSERT INTO editor_bookigroup_members (bookigroup_id, user_id) VALUES ('$pg_group->guid','$pg_guid')";                
                pg_crea_grupo_escritura($pgQuery);
            }
        }
    }
...
/**
 * Agrega los miembros indicados al grupo de escritura de booktype
 */
function pg_add_entity_relationship($guid_one, $relationship, $guid_two) {
    global $CONFIG;
    $guid_one = (int) $guid_one;
    $guid_two = (int) $guid_two;
    // Check for duplicates
    ...
    $pgQuery = "INSERT INTO editor_bookigroup_members (bookigroup_id, user_id) VALUES ('$guid_two','$guid_one')";
    pg_crea_grupo_escritura($pgQuery);
    return false;
}

//agregamos en editor_bookipermission 
function pg_add_entity_relationship_members( $guid_one, $permission, $guid_two) {
    global $CONFIG;
    $guid_one = (int) $guid_one;
    $guid_two = (int) $guid_two;
    //$pgQuery = "INSERT INTO editor_bookigroup_members (bookigroup_id, user_id) VALUES ('$guid_two','$guid_one')";
    //$pgQuery = "INSERT INTO editor_bookipermission ( book_id, group_id, user_id, permission) VALUES ('$book_id', '$guid_two','$guid_one', $permission)";
$pgQuery =
"INSERT INTO editor_bookipermission ( book_id, group_id, user_id, permission) ".
"SELECT e.id as book_id, e.group_id as group_id, '$guid_one' as user_id, '$permission' as permission ".
"FROM editor_book e ".
"WHERE e.group_id='$guid_two'";
...
function relationship_notification_hook($event, $type, $object) {
    ...
    return notify_user($object->guid_two, $object->guid_one, elgg_echo('friend:newfriend:subject', array($user_one->name)), elgg_echo("friend:newfriend:body", array($user_one->name, $user_one->getURL()))
    );
}
