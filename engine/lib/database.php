...
/*
 * Funciones para la base de datos de booktype
 * 
 * 
 */
...
function pg_establish_db_link() {
    // Get configuration, and globalise database link
    global $CONFIG, $pg_dblink;
    // Connect to database
    $pg_dblink = pg_Connect("dbname=$CONFIG->pg_dbname user=$CONFIG->pg_dbuser password=$CONFIG->pg_dbpass host=$CONFIG->pg_dbhost");
    return $pg_dblink;
}

/*insertar usuario a booktype*/
function pg_inserta_usuario($guid, $pgQuery) {
    global $CONFIG,$pg_dblink ;
    $pg_dblink = pg_establish_db_link();
    $result = pg_query($pg_dblink,$pgQuery);
    $result = pg_query($pg_dblink,"INSERT INTO account_userprofile (user_id, mood,description) values ('$guid','','')");
    pg_close($pg_dblink);
}

/*actualiza usuario*/
function pg_actualiza_usuario($pgQuery) {
    global $CONFIG,$pg_dblink ;
    $pg_dblink = pg_establish_db_link();
    $result = pg_query($pg_dblink,$pgQuery);
    pg_close($pg_dblink);
...
function pg_crea_grupo_escritura($pgQuery) {
    global $CONFIG,$pg_dblink ;
    $pg_dblink = pg_establish_db_link();
    $result = pg_query($pg_dblink,$pgQuery);
    pg_close($pg_dblink);
...
function pg_obtener_next_id($pgNombreSecuencia) {
    global $CONFIG,$pg_dblink ;
    $pg_dblink = pg_establish_db_link();
    $result = pg_query($pg_dblink,"SELECT nextval('".$pgNombreSecuencia."')");
    $row = pg_fetch_row($result);
    $id_secuencia = (int)$row[0];
    pg_close($pg_dblink);
    return $id_secuencia;
}

function pg_obtener_secuencia_id($pgSecuencia) {
    global $CONFIG,$pg_dblink ;
    $pg_dblink = pg_establish_db_link();
    $result = pg_query($pg_dblink,'SELECT LAST_VALUE FROM '.$pgSecuencia);
    $row = pg_fetch_row($result);
    $id_secuencia = (int)$row[0];
    pg_close($pg_dblink);
    return $id_secuencia;
...
function pg_crea_bookstatus($pg_id_book, $id_next ) {
    global $CONFIG, $pg_dblink;
    $pg_dblink = pg_establish_db_link();
    $query = "INSERT INTO editor_bookstatus (id, book_id, name, weight) VALUES ('$id_next','$pg_id_book', 'new', '4'); ";
    $query .= "INSERT INTO editor_bookstatus (book_id, name, weight) VALUES ('$pg_id_book', 'needs content', '3'); ";
    $query .= "INSERT INTO editor_bookstatus (book_id, name, weight) VALUES ('$pg_id_book', 'completed', '2'); ";
    $query .= "INSERT INTO editor_bookstatus (book_id, name, weight) VALUES ('$pg_id_book', 'to be proofed', '1');";
    $result = pg_query($pg_dblink,$query);
    pg_close($pg_dblink);
}

function pg_crea_bookversion($pg_id_book, $id_next) {
    global $CONFIG, $pg_dblink;
    $pg_dblink = pg_establish_db_link();
    $query = "INSERT INTO editor_bookversion (id, book_id, major, minor, name, description, created) VALUES ('$id_next','$pg_id_book', '1', '0','initial', '', now())";
    $result = pg_query($pg_dblink,$query);
    pg_close($pg_dblink);
}

function pg_crea_bookhistory($id_book, $pg_owner_id, $id_version) {
    global $CONFIG, $pg_dblink;
    $pg_dblink = pg_establish_db_link();
    $query = "INSERT INTO editor_bookhistory (book_id, version_id, modified, args, user_id, kind) VALUES ('$id_book', '$id_version', now(), '{}', '$pg_owner_id', '10')";
    //$query .= "INSERT INTO editor_bookhistory (book_id, version_id, modified, args, user_id, kind) VALUES ('$pg_id_book', '1', now(), '{\"filename\": \"$pg_file\"}', '$pg_owner_id', '13');";
    $result = pg_query($pg_dblink,$query);
    pg_close($pg_dblink);
}

function pg_crea_bookattachment($id_book, $id_version, $id_status, $data ) {
    global $CONFIG, $pg_dblink;
    $pg_dblink = pg_establish_db_link();
    $pg_Proposal = $data['Proposal'];
    $query = "INSERT INTO editor_attachment (book_id, version_id, attachment, status_id, created) VALUES ('$id_book', '$id_version', '$pg_Proposal', '$id_status', now())";
    $result = pg_query($pg_dblink,$query);
    pg_close($pg_dblink);
}

function pg_crea_editorbook($id_next, $data, $owner_id, $id_group) {
    global $CONFIG, $pg_dblink;
    $pg_dblink = pg_establish_db_link();
    $pg_BookTitle = $data['BookTitle'];
    $pg_url_title = str_replace(" ","-",$pg_BookTitle);
    $pg_description = $data['description'];
    
    $query = "INSERT INTO editor_book (id, url_title, title, group_id, owner_id, license_id, created, published, hidden, permission, description) ";
    $query.= "VALUES ('$id_next', '$pg_url_title','$pg_BookTitle', '$id_group', '$owner_id', '4', now(), now(), 'f', '3', '$pg_description')";
    $result = pg_query($pg_dblink,$query);
    pg_close($pg_dblink);
}

function pg_actualiza_editorbook($id_book, $id_version, $id_status) {
    global $CONFIG, $pg_dblink;
    $pg_dblink = pg_establish_db_link();
    $query = "UPDATE editor_book SET status_id = '$id_status', version_id = '$id_version' WHERE id = '$id_book'";
    $result = pg_query($pg_dblink,$query);
    pg_close($pg_dblink);
}

function pg_obtener_titulo_url_editorbook($id_group) {
    global $CONFIG, $pg_dblink;
    $pg_dblink = pg_establish_db_link();
    $query = "SELECT title, url_title FROM editor_book WHERE group_id='$id_group'";
    $result = pg_query($pg_dblink,$query);
    //pg_close($pg_dblink);
    return $result;
}
