...
function create_user_entity($guid, $name, $username, $password, $salt, $email, $language, $code) {
	...
	$code = sanitise_string($code);
        //obtenemos la accion que se esta ejecutando para 
        //prevenir la actualizacion incorrecta del password en booktype
        $pg_action = get_input("action");
	...
	if ($row) {
		...
		if ($exists = get_data_row($query)) {
			...
			$result = update_data($query);
                        ...
                        if ($pg_action != 'logout') {
                        //$salt5 = substr($salt, 0, 5);
                        $salt5 = '12345'; //substr($salt, 0, 5);
                        $pg_query = "UPDATE auth_user SET username='$username', 
                                    first_name='$name', password='sha1$'||'$salt5'||'$'||ENCODE(DIGEST('$salt5'||'$pg_passwd', 'sha1'), 'hex'),
                                    email='$email' WHERE id = $guid";
                        pg_actualiza_usuario($pg_query);
                        }
                        if ($result != false) {
			...
			$result = insert_data($query);
                        if ($pg_action != 'logout') {
                            $salt5 = '12345';//substr($salt, 0, 5);
                            $pg_query = "INSERT into auth_user (id, username, first_name, last_name, email, password, is_staff, is_active, is_superuser, last_login, date_joined) values ('$guid', '$username', '$name', '', '$email','sha1$'||'$salt5'||'$'||ENCODE(DIGEST('$salt5'||'$pg_passwd', 'sha1'), 'hex'),'f', 't', 'f', now(), now() )";
                            pg_inserta_usuario($guid, $pg_query);
                        }
                        if ($result !== false) {
				$entity = get_entity($guid);
